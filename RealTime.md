...
Flow
Browser: getUserMedia → live <video> preview. On the same cadence as the video pipeline (frame_interval_sec from shared/pipeline_public_constants.json, currently 20s), it runs OpenCV.js preprocessing that mirrors GazeInference._extract_eye_region and PostureInference._detect_person (Haar face/eyes, HOG person, same resize sizes as pipeline_public_constants.json / Python).
...
First recording: loads OpenCV.js from https://docs.opencv.org/4.8.0/opencv.js and Haar XML from GitHub; first run can be slow and needs network access.
